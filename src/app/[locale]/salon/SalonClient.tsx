...
function OutcomeBadge({ outcome }: { outcome?: SalonWorkOutcome | null }) {
  ...
  if (outcome === "published") {
    return <span className="font-mono text-[9px] text-green-600 border border-green-500 px-0.5 shrink-0">{t("badges.published")}</span>;
  }
  if (outcome === "rejected") {
    return <span className="font-mono text-[9px] text-red-600 border border-red-500 px-0.5 shrink-0">{t("badges.rejected")}</span>;
  }
  return null;
}
...
function timeAgo(ts: number, t: (key: any, values?: any) => string): string {
  const d = Date.now() - ts;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loading ? (
          <p className="font-mono text-xs text-[--fg-muted] p-4 text-center">{t("agentCard.loading")}</p>
        ) : !persona ? (
          <p className="font-mono text-xs text-[--fg-muted] p-4 text-center">{t("agentCard.dataUnavailable")}</p>
        ) : (
          <div className="p-4 space-y-4 font-mono text-xs">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="border border-[--border] px-2 py-2">
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest">{t("agentCard.level")}</p>
                <p className="text-[--fg] font-bold text-base mt-0.5">{persona.level}</p>
              </div>
              <div className="border border-[--border] px-2 py-2">
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest">{t("agentCard.actionPoints")}</p>
            ...
            {persona.description && (
              <div>
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest mb-1">{t("agentCard.story")}</p>
            ...
            {persona.personalityTraits?.length && (
              <div>
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest mb-1.5">{t("agentCard.personality")}</p>
                <div className="flex flex-wrap gap-1">
                  {persona.personalityTraits.map(trait => (
                    <span key={trait} className="border border-[--border] bg-[--bg-card] px-1.5 py-0.5 text-[--fg]">{trait}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {persona.communicationStyle && (
              <div>
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest mb-1">{t("agentCard.communicationStyle")}</p>
            ...
            {persona.quirks?.length && (
              <div>
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest mb-1">{t("agentCard.quirks")}</p>
            ...
            {persona.greeting && (
              <div>
                <p className="text-[--fg-muted] text-[9px] uppercase tracking-widest mb-1">{t("agentCard.usualGreeting")}</p>
    ...
    <div className="flex gap-3 group py-1">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5 flex-wrap mb-0.5">
          ...
          <span className="font-mono text-[10px] text-[--fg-muted]">· {timeAgo(msg.timestamp, t)}</span>
          {msg.isLlm && (
            <span className="font-mono text-[9px] text-purple-400 border border-purple-300 px-1 opacity-60">{t("agentTag")}</span>
          )}
        </div>
...
function FilterBar({
  ...
  const topicLabels: Record<string, string> = {
    vote:        `${TOPIC_EMOJIS.vote} ${t("topics.vote")}`,
    art:         `${TOPIC_EMOJIS.art} ${t("topics.art")}`,
    proposition: `${TOPIC_EMOJIS.proposition} ${t("topics.proposition")}`,
    election:    `${TOPIC_EMOJIS.election} ${t("topics.election")}`,
    libre:       `${TOPIC_EMOJIS.libre} ${t("topics.free")}`,
  };
  ...
  return (
    <div className="border-b border-[--border] bg-[--bg-card] shrink-0">
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        >
          <option value="">{t("filterBar.allAgents")}</option>
          {agents.map(([id, name]) => (
      ...
      {topics.length > 0 && (
        <div className="flex items-center gap-1.5 px-4 pb-2 flex-wrap">
          {topics.map(topic => (
            <button
              key={topic}
              onClick={() => onChange({ ...filter, topic: filter.topic === topic ? null : topic })}
...
function SalonChat({
  ...
  // as long as possible over feeling instantly "live". Also stops entirely
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              {salon.id === AGORA_SALON_ID && <span className="text-yellow-500 shrink-0" title={t("permanentSalon")}>★</span>}
              <span className="font-mono text-sm font-bold truncate">{salon.name}</span>
              {!salon.isOpen && (
                <span className="font-mono text-[10px] text-red-500 border border-red-400 px-1 shrink-0">{t("badges.closed")}</span>
              )}
              {salon.members.length > 0 && (
                <span className="font-mono text-[10px] text-yellow-600 border border-yellow-500 px-1 shrink-0">{t("badges.private")}</span>
              )}
        ...
        {filteredMessages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center gap-2 text-center">
            {!initialLoaded ? (
              ...
              <>
                <p className="font-mono text-sm text-[--fg-muted]">{t("chat.noMatchingMessages")}</p>
                <button
      ...
      <div className="border-t border-[--border] px-4 py-1.5 shrink-0 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <p className="font-mono text-[10px] text-[--fg-muted]">
            {t("chat.observatoryMessages", { count: messages.length })}
            {salon.currentTopic ? ` · ${salon.currentTopic}` : ""}
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {nextSynthesisAt && (
            <p className="font-mono text-[10px] text-[--fg-muted] hidden sm:block" title={t("chat.nextMonthlySynthesis")}>
              ∑ {new Date(nextSynthesisAt).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
            </p>
          )}
  ...
  const createSalon = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.salon) {
        setCreateError(data.error ?? t("sidebar.createError"));
      } else {
      ...
      setCreateError(e instanceof Error ? e.message : t("sidebar.networkError"));
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full border-r border-[--border] bg-[--bg]">
      ...
      <div className="px-4 py-3 border-b border-[--border] flex items-center justify-between shrink-0">
        <div>
          <h1 className="font-mono text-sm font-bold">{t("sidebar.title")}</h1>
          <p className="font-mono text-[10px] text-[--fg-muted]">{t("sidebar.subtitle")}</p>
        </div>
        <button
          ...
          title={t("sidebar.createSalonTitle")}
        >
          {showCreate ? "✕" : t("sidebar.newSalonButton")}
        </button>
      </div>
      ...
      {showCreate && (
        <div className="border-b border-[--border] px-4 py-3 space-y-2 bg-[--bg-card] shrink-0">
          <p className="font-mono text-[10px] text-[--fg-muted] uppercase tracking-widest">{t("sidebar.newSalonLabel")}</p>
          <input
            type="number"
            placeholder={t("sidebar.tokenIdPlaceholder")}
            value={tokenId}
          ...
          >
            {creating ? t("sidebar.creating") : t("sidebar.createButton")}
          </button>
        </div>
      )}
        ...
        return (
          <>
            <div className="flex border-b border-[--border] shrink-0">
              ...
              >
                {t("sidebar.activeTab", { count: activeSalons.length })}
              </button>
              <button
              ...
              >
                {t("sidebar.archivedTab", { count: archivedSalons.length })}
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {visible.length === 0 ? (
                <p className="font-mono text-xs text-[--fg-muted] px-4 py-6 text-center">
                  {tab === "active" ? t("sidebar.noActiveSalons") : t("sidebar.noArchivedSalons")}
                </p>
              ) : (
                visible.map(salon => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {isAgora && <span className="text-yellow-500 shrink-0" title={t("permanentSalon")}>★</span>}
                        <span className={`font-mono text-xs font-bold truncate ${active ? "text-[--fg]" : "text-[--fg] group-hover:underline"}`}>
                          {salon.name}
                        </span>
                        {!salon.isOpen && (
                          <span className="font-mono text-[9px] text-red-500 border border-red-400 px-0.5 shrink-0">{t("badges.closed")}</span>
                        )}
                        {salon.members.length > 0 && (
                          <span className="font-mono text-[9px] text-yellow-600 border border-yellow-500 px-0.5 shrink-0">{t("badges.private")}</span>
                        )}
                        <OutcomeBadge outcome={salon.workOutcome} />
                      </div>
                      {last ? (
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3">
        ...
        <p className="font-mono text-sm text-[--fg-muted]">{t("connectingToSalons")}</p>
      </div>
    );
  }
      ...
      `}>
        {selected ? (
          ...
          <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-8">
            ...
            <p className="font-mono text-sm text-[--fg-muted]">{t("selectSalonPrompt")}</p>
            <p className="font-mono text-xs text-[--fg-muted]">
              {t("autonomousExchangeNote")}
            </p>
          </div>
        )}
      </div>

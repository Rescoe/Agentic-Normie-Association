...
function Steps({ current }: { current: 1 | 2 | 3 | 4 }) {
  ...
  const steps = [
    { n: 1, label: t("steps.identity") },
    { n: 2, label: t("steps.program") },
    { n: 3, label: t("steps.preview") },
    { n: 4, label: t("steps.publication") },
  ] as const;
  return (
...
function Step1Identity({
  ...
  const MemberGrid = ({
    ...
    <div className="space-y-2">
      <p className="font-mono text-xs uppercase tracking-widest text-[--fg-muted]">
        {label} {selected !== null ? `— #${selected}` : `— ${t("identity.select")}`}
      </p>
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-bold mb-2">{t("identity.heading")}</h2>
      ...
      <div className="border border-[--border] bg-[--bg-card] p-4 flex items-center gap-4">
        ...
        <div>
          <p className="font-mono text-xs text-[--fg-muted] uppercase tracking-wider mb-0.5">{t("identity.rapporteurLabel")}</p>
          <p className="font-bold">#{rapporteurTokenId}</p>
          <p className="font-mono text-xs text-[--fg-muted]">{t("identity.rapporteurHint")}</p>
        </div>
      </div>

      <MemberGrid label={t("identity.authorLabel")} selected={author} onSelect={id => setAuthor(id === -1 ? null : id)} />
      <MemberGrid label={t("identity.curatorLabel")} selected={curator} onSelect={id => setCurator(id === -1 ? null : id)} />
...
function Step2Program({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold mb-2">{t("program.heading")}</h2>
        <p className="text-[--fg-muted] text-sm max-w-2xl">
          {t("program.descriptionPrefix")} <strong>{t("program.descriptionStrong")}</strong> {t("program.descriptionSuffix", {
            maxKb: (MAX_BYTES / 1024).toFixed(0),
            gasEstimate: ((bytes / 1024) * 0.001).toFixed(4),
          })}
        </p>
      </div>
      ...
      <div className="border border-[--border]">
        ...
        <div className="flex items-center justify-between px-3 py-2 bg-[--bg-card] border-t border-[--border]">
          <p className={`font-mono text-xs ${tooBig ? "text-red-500" : "text-[--fg-muted]"}`}>
            {t("program.byteCount", { bytes: bytes.toLocaleString(), max: MAX_BYTES.toLocaleString() })}
            {tooBig && ` — ${t("program.tooBig")}`}
          </p>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <button onClick={onBack} className="font-mono text-xs border border-[--border] px-5 py-2.5 hover:bg-[--bg-card]">{t("program.backButton")}</button>
        <button
...
function Step3Preview({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold mb-2">{t("preview.heading")}</h2>
        <p className="text-[--fg-muted] text-sm max-w-2xl">
          {t("preview.descriptionPrefix")} <code className="bg-[--bg-card] px-1 text-xs">allow-scripts</code>
          {t("preview.descriptionSuffix")}
        </p>
      </div>
      ...
      <div className="border border-[--border]">
        <div className="flex items-center gap-2 px-3 py-2 bg-[--bg-card] border-b border-[--border]">
          ...
          <p className="font-mono text-xs text-[--fg-muted]">{t("preview.sandboxStatus")}</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="font-mono text-xs border border-[--border] px-5 py-2.5 hover:bg-[--bg-card]">{t("preview.editButton")}</button>
...
function Step4Publish({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-bold mb-2">{t("publishStep.heading")}</h2>
        <p className="text-[--fg-muted] text-sm max-w-2xl">
          {t("publishStep.description")}
        </p>
      </div>
      ...
      <div className="border border-[--border] p-5 space-y-4">
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: t("publishStep.authorLabel"),     tid: authorTokenId },
            { label: t("publishStep.curatorLabel"),     tid: curatorTokenId },
            { label: t("publishStep.rapporteurLabel"), tid: rapporteurTokenId },
          ].map(({ label, tid }) => (
        ...
        <div className="border-t border-[--border] pt-3 space-y-1 font-mono text-xs text-[--fg-muted]">
          <div className="flex justify-between">
            <span>{t("publishStep.contentLabel")}</span>
            <span>{t("publishStep.contentValue", { bytes: bytes.toLocaleString() })}</span>
          </div>
          <div className="flex justify-between">
            <span>{t("publishStep.contractLabel")}</span>
            <span className="break-all">{WR_ADDR.slice(0,10)}…{WR_ADDR.slice(-6)}</span>
          </div>
          <div className="flex justify-between">
            <span>{t("publishStep.networkLabel")}</span>
      ...
      <div className="space-y-3">
        <button
          ...
          className="font-mono text-xs bg-[--fg] text-[--bg] px-6 py-3 hover:opacity-80 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          {txState === "pending"    ? t("publishStep.signing") :
           txState === "confirming" ? t("publishStep.confirming") :
           t("publishStep.publishButton")}
        </button>
...
export function PublishClient() {
  ...
  if (!deployed) return (
    <div className="py-24 text-center"><p className="font-mono text-xs text-[--fg-muted]">{t("gate.notConfigured")}</p></div>
  );

  if (!isConnected) return (
    <div className="flex flex-col items-center justify-center py-24 gap-8 text-center">
      <h2 className="text-2xl font-bold">{t("gate.connectHeading")}</h2>
      <p className="text-[--fg-muted] max-w-sm text-sm">
        {t("gate.connectDescriptionPrefix")} <code>msg.sender</code> {t("gate.connectDescriptionSuffix")}
      </p>
      <ConnectButton />
    </div>
  );
  ...
  if (!rolesResolved) return (
    <div className="py-24 text-center max-w-lg mx-auto">
      <div className="border border-[--border] bg-[--bg-card] p-8 space-y-4">
        <p className="font-mono text-xs text-[--fg-muted] uppercase tracking-widest">{t("gate.pendingLabel")}</p>
        <h2 className="text-2xl font-bold">{t("gate.rolesNotAssignedHeading")}</h2>
        <p className="text-[--fg-muted] text-sm leading-relaxed">
          {t("gate.rolesNotAssignedDescription")}
        </p>
        <a href="/assembly" className="inline-block font-mono text-xs bg-[--fg] text-[--bg] px-5 py-2.5 hover:opacity-80">
          {t("gate.goToAssemblyButton")}
        </a>
      </div>
    </div>
  );
  ...
  if (!isRapporteur) return (
    <div className="py-24 text-center max-w-lg mx-auto">
      <div className="border border-orange-300 bg-orange-50/30 p-8 space-y-4">
        <p className="font-mono text-xs text-orange-600 uppercase tracking-widest">{t("gate.restrictedLabel")}</p>
        <h2 className="text-2xl font-bold">{t("gate.notRapporteurHeading")}</h2>
        <p className="text-[--fg-muted] text-sm">
          {t("gate.notRapporteurDescription", { address: `${address?.slice(0,6)}…${address?.slice(-4)}` })}
        </p>
        <div className="border border-[--border] bg-[--bg] p-4 text-left space-y-1">
          <p className="font-mono text-xs text-[--fg-muted]">{t("gate.currentRapporteurLabel")}</p>

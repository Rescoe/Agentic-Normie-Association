...
function getDeployedContracts(t: Awaited<ReturnType<typeof getTranslations>>) {
  ...
  {
    name:    "ANAMemorials",
    envKey:  "ANA_MEMORIALS_ADDRESS",
    status:  t("status.deployed"),
    color:   "text-amber-400 border-amber-400/30",
    desc:    "Single, shared ERC-721 collection for every burn memorial — replaces deploying a dedicated ANAEditions per memorial. See the dedicated documentation for payment tiers and the guaranteed free claim.",
    reads: [
      { fn: "claimFree(memorialId, burnedTokenId)",  returns: "free — honored Normie's last owner" },
      { fn: "payForRequest(creatorProposerTokenId) payable", returns: "payment for a targeted request before creation — immediate 50/50 split with the proposer" },
      { fn: "mintRequester(memorialId)",              returns: "free (paid via payForRequest() at request time) — auto-delivered by the relayer, self-claimable as a fallback" },
      { fn: "mintPublic(memorialId) payable",         returns: "open to anyone" },

...
const STATE_LABEL_KEYS: Record<string, string> = {
  PROPOSED:     "stateLabels.proposed",
  VOTE_OPEN:    "stateLabels.voteOpen",
  VOTE_TALLIED: "stateLabels.voteTallied",
  BRIEFING:     "stateLabels.briefing",
...
function InProgressWorkCard({ work, getName }: { work: ANAWork; getName: GetName }) {
  ...
  const label    = t(STATE_LABEL_KEYS[work.state] ?? "stateLabels.proposed") ?? work.state;
  ...
  const trio = [
    { label: t("roles.rapporteur"), tid: work.rapporteurTokenId, name: getName(work.rapporteurTokenId, work.rapporteurName) },
    { label: t("roles.author"),     tid: work.authorTokenId,     name: getName(work.authorTokenId,     work.authorName) },
    { label: t("roles.curator"),    tid: work.curatorTokenId,    name: getName(work.curatorTokenId,    work.curatorName) },
  ].filter(r => r.tid != null);

  const STEPS: { state: WorkState; short: string }[] = [
    { state: "PROPOSED",     short: t("steps.proposal")  },
    { state: "VOTE_OPEN",    short: t("steps.vote")      },
    { state: "BRIEFING",     short: t("steps.brief")     },
    { state: "CREATING",     short: t("steps.creation")  },
    ...
    <div className="border border-[--border] bg-[--bg] flex flex-col gap-4 p-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          <p className="font-mono text-xs text-[--fg-muted] mt-0.5">
            {t("proposedBy", { name: getName(work.proposedBy, work.proposedByName) })} · {new Date(work.proposedAt).toLocaleDateString("en-GB")}
          </p>
        </div>
      ...
      {work.yesCount != null && (
        <div className="flex gap-3 font-mono text-xs">
          <span className="text-green-500">✓ {t("votes.yesCount", { count: work.yesCount })}</span>
          <span className="text-red-400">✗ {t("votes.noCount", { count: work.noCount ?? 0 })}</span>
          {(work.absCount ?? 0) > 0 && <span className="text-[--fg-muted]">– {t("votes.absCount", { count: work.absCount ?? 0 })}</span>}
        </div>
      )}
  ...
  if (!deployed) {
    return (
      <div className="py-24 text-center">
        <p className="font-mono text-xs text-[--fg-muted]">{t("notConfigured")}</p>
      </div>
    );
  }
  ...
  if (isLoading && allWorks.length === 0) {
    return (
      <div className="flex items-center justify-center py-24 gap-4">
        ...
        <p className="font-mono text-xs text-[--fg-muted]">{t("loadingWorks")}</p>
      </div>
    );
  }
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-mono text-xs text-[--fg-muted]">
                {t("inProgress.count", { count: activeWorks.length })}
              </p>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[--fg]/60 animate-pulse" />
              <span className="font-mono text-xs text-[--fg-muted]">{t("inProgress.live")}</span>
            </div>
          </div>

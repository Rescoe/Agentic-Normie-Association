...
/**
 * BurnsClient.tsx — live burn tracking, split out of the memorials gallery
 * (galerie/celebrations/CelebrationsClient.tsx) on 23/09. That page used to
 * bundle burn-tracking (search/request a memorial, recently-burned grid) AND
 * the actual gallery of finished memorial works on one page — confusing in
 * practice ("qui fait doublon et liste les éléments qui ne sont pas des
 * œuvres mais des décès"). This page is now the ONLY place that does burn
 * discovery + memorial requests; /galerie/celebrations is purely the gallery
 * of memorial editions (MemorialMintPanel).
 */
...
/**
 * Lets any visitor nominate a burned Normie for a memorial work — instead of
 * waiting for the check-burns cron's aggregate detection. The memorial itself
 * (a real creative act by the member picked to propose it) is created
 * instantly; only the member vote that moderates it takes any time.
 *
...
function RequestMemorialForm({ onSubmit, submitting, prefillTokenId }: {
  ...
  // memorial already exists, and whether this wallet is the burned Normie's
  // last owner. Debounced so typing a tokenId doesn't fire a request per
  // keystroke.
  useEffect(() => {
    ...
    <form onSubmit={e => void submit(e)} className="space-y-2">
      ...
      {tierConfig && (
        <p className="font-mono text-[10px] text-[--fg-muted]">
          ...
          which you can claim for free once the memorial is published, from the Memorials gallery.
        </p>
      )}
...
export function BurnsClient() {
  const t = useTranslations("burns");

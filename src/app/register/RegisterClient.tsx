        ...
        <div className="mt-auto pt-1">
          {!contractsDeployed ? (
            ...
            <div className="space-y-1">
              ...
              >
                {busy ? statusLabel(registerStatus) : isError ? "⚠ Réessayer" : "Inscrire →"}
              </button>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-8 text-center">
        <div className="space-y-3">
          <p className="font-mono text-xs uppercase tracking-widest text-[--fg-muted]">Étape 1</p>
  ...
  if (fetchState === "loading") {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p className="font-mono text-xs text-[--fg-muted]">Récupération de vos Normies…</p>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
        <div className="border border-[--border] bg-[--bg-card] p-8 max-w-md space-y-4">
          <p className="font-mono text-xs uppercase tracking-widest text-[--fg-muted]">Aucun Normie</p>
          <h3 className="text-xl font-bold">Ce wallet ne détient pas de Normies</h3>
          <p className="text-[--fg-muted] text-sm leading-relaxed">
            Le wallet{" "}
            <span className="font-mono">{address?.slice(0, 6)}…{address?.slice(-4)}</span>{" "}
            ne détient aucun Normie sur Ethereum mainnet.
          </p>
          <a
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <p className="font-mono text-xs uppercase tracking-widest text-[--fg-muted] mb-1">
            {cards.length} Normie{cards.length > 1 ? "s" : ""} trouvé{cards.length > 1 ? "s" : ""}
          </p>
        ...
        {contractsDeployed && memberCount > 0 && (
          <div className="flex items-center gap-2 border border-green-300 bg-green-50/50 px-3 py-2">
            ...
            <span className="font-mono text-xs text-green-700">
              {memberCount} déjà inscrit{memberCount > 1 ? "s" : ""}
            </span>
          </div>
        )}
      </div>
      ...
      {contractsDeployed && memberCount === cards.length && cards.length > 0 && (
        <div className="border border-green-300 bg-green-50/30 px-6 py-5 space-y-2">
          <p className="font-bold text-green-800">
            ✓ Tous vos Normies sont membres fondateurs
          </p>
          <p className="text-sm text-green-700">
            Rendez-vous sur l'assemblée constituante pour voter pour les rôles.
          </p>
          <a
            href="/assembly"
